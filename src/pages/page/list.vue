<template>
  <div class="app-container">
    ...
    <el-tabs type="card" v-model="selectedCategoryName" @tab-remove="deletePageCategory" @tab-click="onTabClick">
      <el-tab-pane
        v-for="category in pageCategoryList"
        :key="category.id"
        :label="category.name"
        :name="category.name"
        :closable="category.name !== '全部'"
      />
    </el-tabs>
    <!--page列表-->
    <div>
      <el-table :data="pageList" highlight-current-row border>
        <el-table-column label="分类" align="center">
          <template slot-scope="{ row }">
            ...
            >
              <el-option
                v-for="category in pageCategoryListWithoutTotal"
                :key="category.id"
                :value="category.id"
                :label="category.name"
              />
    ...
    <div>
      <pagination
        v-show="total > 0"
        :total="total"
        :page.sync="queryPageListForm.pageNum"
        :limit.sync="queryPageListForm.pageSize"
